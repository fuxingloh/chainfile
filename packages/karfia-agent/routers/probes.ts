...
async function query(definition: KarfiaDefinition, probeType: ProbeType) {
...
function createProbeFunction(
  ...
  switch (protocol) {
    ...
    case 'HTTPS JSON-RPC 2.0':
      return createProbeFunctionHttpJsonRpc(containerName, endpoint as ContainerEndpointHttpJsonRpc, probeType);
    case 'HTTP REST':
    case 'HTTPS REST':
      return createProbeFunctionHttp(containerName, endpoint as ContainerEndpointHttpRest, probeType);
    default:
...
function createProbeFunctionHttp(
  containerName: string,
  endpoint: ContainerEndpointHttpRest,
  probeType: ProbeType,
): ProbeFunction | undefined {
...
function createProbeFunctionHttpJsonRpc(
  containerName: string,
  endpoint: ContainerEndpointHttpJsonRpc,
  probeType: ProbeType,
): ProbeFunction | undefined {
  const probe = endpoint.probes?.[probeType];
  if (probe === undefined) {
...
function getHttpAuthorizationHeaders(auth: ContainerEndpointHttpAuthorization): Record<string, string> {
 ...
 * Look for `${KEY}` injected through `KARFIA_ENVIRONMENT_KEY`.
 *
...
function resolveValue(value: string | EnvironmentReference): string {
  ...
  return process.env[`KARFIA_ENVIRONMENT_${value.key}`] ?? '';
}
