...
import { synthEnv } from 'chainfile/env';
import { Chainfile, Container, validate } from 'chainfile/schema';
...
export class Synthesizer {
  ...
  constructor(chainfile: Chainfile | any) {
    validate(chainfile);
  ...
  public synthCompose(): string {
    return [
      ...
      `# Chainfile ID: ${this.chainfile.id}`,
      `# Chainfile Name: ${this.chainfile.name}`,
      `# Chainfile CAIP-2: ${this.chainfile.caip2}`,
      '',
      yaml.dump(
        {
          name: this.chainfile.id.replaceAll(/[^a-z0-9_-]/g, '_'),
          services: {
  ...
  private createChainfileAgent(): Record<'chainfile-agent', object> {
    const chainfileJson = JSON.stringify({
      ...this.chainfile,
      $schema: undefined,
    }).replaceAll('$', '$$$');
